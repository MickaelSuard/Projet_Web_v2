...
import { ITEMS_PER_PAGE, MOCK_CAPSULES, MOCK_USER } from "./home/data/mockData";
import { useToast } from "./home/hooks/useToast";
import type { Capsule } from "./home/types";
...
export default function HomePage() {
  ...
  const filteredCapsules = capsules.filter(
    (capsule) =>
      capsule.description.toLowerCase().includes(normalizedSearch) ||
      capsule.username.toLowerCase().includes(normalizedSearch),
  );
  ...
  const toggleLike = (id: number) => {
    ...
    setCapsules((currentCapsules) =>
      currentCapsules.map((capsule) =>
        capsule.id === id ? { ...capsule, isLiked } : capsule,
      ),
    );
  };
  ...
  const toggleBookmark = (id: number) => {
    ...
    addToast(isBookmarked ? "🔖 Sauvegardé" : "Signet retiré");

    setCapsules((currentCapsules) =>
      currentCapsules.map((capsule) =>
        capsule.id === id ? { ...capsule, isBookmarked } : capsule,
      ),
    );
  };
        ...
        {paginatedCapsules.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {paginatedCapsules.map((capsule) => (
              <CapsuleCard key={capsule.id} capsule={capsule} onToggleLike={toggleLike} onToggleBookmark={toggleBookmark} />
            ))}
          </div>
        )}
